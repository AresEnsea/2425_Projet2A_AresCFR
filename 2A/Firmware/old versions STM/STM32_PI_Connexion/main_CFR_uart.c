...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        // Transmettre les données reçues pour débogage
        ...
        // Vider le buffer pour éviter les lectures répétées
        //HAL_UART_Abort(&hlpuart1);
        HAL_UART_Receive_IT(&hlpuart1, (uint8_t*)rx_data, sizeof(rx_data)); // Démarrer la réception

        // Traiter les commandes reçues

        // Réinitialiser le buffer pour éviter la confusion lors de la prochaine réception
        //memset(rx_data, 0, sizeof(rx_data)); // Optionnel : pour vider le buffer
    }
}
