...
class SerialBagListener(Node):
    ...
    def command_callback(self, msg):
        ...
        if self.motor_stopped:
            # Modify first 10 characters to '0'
            if len(command) >= 10:
                modified_command = '0' * 10 + command[10:]
            else:
                # Handle shorter commands by padding with '0's
                modified_command = '0' * 10 + command
            self.get_logger().info(f'Motor stopped: sending modified command: {modified_command}')
